...
import type { DigitalTwinResponse, Facility, Road } from '../types/api'

const facilityStyles = {
  hospital: { color: '#dc2626', label: 'Hospital' },
  clinic: { color: '#2563eb', label: 'Clinic' },
  school: { color: '#16a34a', label: 'School' },
  public: { color: '#7c3aed', label: 'Public facility' },
}
...
function MapViewport({ twin }: { twin: DigitalTwinResponse | null }) {
  ...
  useEffect(() => {
    if (!twin) return
    const points = twin.roads.flatMap((road) =>
      road.coordinates.map(([longitude, latitude]) => [latitude, longitude] as [number, number]),
    )
...
function FacilityMarker({ facility, category }: { facility: Facility; category: keyof typeof facilityStyles }) {
  ...
  return (
    <CircleMarker
      ...
      radius={8}
      pathOptions={{ color: '#ffffff', fillColor: style.color, fillOpacity: 1, weight: 2 }}
    >
      <Popup>
...
function RoadLine({ road, selected, onSelect }: { road: Road; selected: boolean; onSelect: (edgeId: string) => void }) {
  ...
  return (
    ...
    >
      <Popup>
        <p className="font-semibold">{road.name ?? 'Unnamed road'}</p>
        <p className="text-slate-600">{road.length_meters.toFixed(0)} m · {road.travel_time_seconds.toFixed(0)} s</p>
        <p className="mt-1 text-indigo-600">Select this road for closure simulation.</p>
      </Popup>
    </Polyline>
  )
}
...
interface Props {
  twin: DigitalTwinResponse | null
  selectedEdgeId: string | null
  onEdgeSelect: (edgeId: string) => void
}

export function DigitalTwinMap({ twin, selectedEdgeId, onEdgeSelect }: Props) {
  const center: [number, number] = twin?.nodes[0]
    ? [twin.nodes[0].latitude, twin.nodes[0].longitude]
    : [10.0159, 76.3419]

  return (
    <MapContainer center={center} zoom={13} zoomControl scrollWheelZoom className="h-[620px] w-full">
      ...
      />
      {twin?.roads.map((road) => (
        <RoadLine key={road.id} road={road} selected={road.id === selectedEdgeId} onSelect={onEdgeSelect} />
      ))}
